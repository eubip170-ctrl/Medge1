...
def _calmar(eq: pd.Series, rp: pd.Series) -> float:
    ...
    if not np.isfinite(c) or not np.isfinite(mdd) or mdd >= 0:
        return float("nan")
    den = abs(mdd)
    return float(c / den) if den > 1e-12 else float("nan")
...
def _apply_holding_constraints(w: pd.Series, max_holdings: int, min_w_threshold: float) -> pd.Series:
    ...
    if max_holdings and max_holdings > 0 and int(ww.ne(0).sum()) > int(max_holdings):
...
def _plot_compare_base100(eq_a: pd.Series, eq_b: pd.Series, name_a: str, name_b: str) -> None:
    ...
    if a.empty or b.empty:
        return
    a100 = (a / float(a.iloc[0])) * 100.0
    b100 = (b / float(b.iloc[0])) * 100.0
    df = pd.DataFrame({name_a: a100, name_b: b100}).dropna()
    if df.empty:
        return
...
def _parse_sector_caps(txt: str) -> Dict[str, float]:
    d: Dict[str, float] = {}
    if not txt or not txt.strip():
        return d
    for pair in re.split(r"[;,]+", txt.strip()):
...
def _resolve_args(*args: Any, **kwargs: Any) -> Tuple[Dict[str, dict], str, Any]:
    """
    Compatibilità:
      - render_optimization_page(runs, pid, state)
      - render_optimization_page(res=..., pid=..., state=...)
      - qualsiasi combinazione (non deve crashare)
    """
    runs = kwargs.get("runs")
    if runs is None:
        runs = kwargs.get("res")
    if runs is None and len(args) >= 1:
        runs = args[0]
    if runs is None:
    ...
    if state is None and len(args) >= 3:
        state = args[2]
    if state is None:
        state = st.session_state

    return runs, str(pid), state


def _call_optimize_weights_safe(
    ...
    """
    Prova varie firme di optimize_weights per evitare TypeError
    se la tua implementazione ha parametri diversi.
    """
    # tentativo 1: firma "ricca"
    try:
    ...
    # tentativo 2: senza vincoli settoriali
    try:
        return optimize_weights(
            rets_assets=rets_win,
            rf_annual=float(rf_annual),
    ...
    # tentativo 3: firma più vecchia (posizionale)
    return optimize_weights(rets_win, float(rf_annual), str(objective), bounds)
...
def render_optimization_page(*args: Any, **kwargs: Any) -> None:
    runs, pid, state = _resolve_args(*args, **kwargs)

    st.subheader("Ottimizzazione — What-if + vincoli + confronto")

    if not runs:
        st.info("Definisci almeno 1 portafoglio e premi **Run / Update (ALL)**.")
        return

    if not pid or pid not in runs:
        pid = list(runs.keys())[0]

    r = runs.get(pid, {})
    prices = r.get("prices", None)

    if prices is None or getattr(prices, "empty", True):
        st.warning("Questo portafoglio non ha prices validi.")
        return
    ...
    try:
        ...
        g = st.session_state.get(SS_GLOBAL, {}) or {}

    rf_global = float(g.get("rf_annual", 0.01))
    rachev_alpha = float(g.get("rachev_alpha", 0.05))
    initial_capital = float(g.get("initial_capital", 100000.0))

    prices_now = _safe_prices(prices)

    cols_px = list(prices_now.columns)
    if not cols_px:
        st.warning("Prices non hanno colonne (tickers).")
        return

    # current weights
    w_raw = r.get("weights", None)
    if w_raw is None:
    ...
    with cL:
        ...
        OBJECTIVES = ["max_sharpe", "max_sortino", "min_cvar95", "min_cvar99"]
        objective = st.selectbox("Objective", OBJECTIVES, index=0, key=f"opt_objective__{pid}")
        ...
        with cc3:
            rf_opt = st.number_input(
                "RF annual (optimization)",
                value=rf_global,
                step=0.001,
                format="%.3f",
                key=f"opt_rf__{pid}",
        ...
        with c5:
            min_thr = st.number_input(
                ...
                step=0.005,
                format="%.3f",
                key=f"opt_min_thr__{pid}",
            )
        with c6:
            turnover_cap = st.number_input(
                "Turnover cap (0=off)",
                min_value=0.0,
                max_value=2.0,
                value=0.0,
                step=0.05,
                format="%.2f",
                key=f"opt_turnover_cap__{pid}",
            )

        tcost_bps = st.number_input(
            "Transaction cost (bps)",
            min_value=0.0,
            max_value=200.0,
            value=0.0,
            step=1.0,
            key=f"opt_tcost__{pid}",
        )

        with st.expander("🏷️ Sector constraints (optional)", expanded=False):
            sector_map_text = st.text_area(
                "Sector map (Ticker:Sector)",
                value="",
                placeholder="AAPL:Tech, MSFT:Tech, XOM:Energy",
                key=f"opt_sector_map__{pid}",
            )
            sector_caps_text = st.text_area(
                "Sector caps (Sector=cap)",
                value="",
                placeholder="Tech=0.6, Energy=0.3",
    ...
    with cR:
        ...
        df_cur = pd.DataFrame({"Weight": w_current}).sort_values("Weight", ascending=False)
        st.dataframe(df_cur, use_container_width=True, height=260)

        rets_all = _rets_assets(prices_now)
        rets_win = rets_all.loc[
            (rets_all.index >= pd.to_datetime(p_start)) & (rets_all.index <= pd.to_datetime(p_end))
        ].copy()
    ...
    if do_opt:
        try:
            ...
            w_opt_raw = _call_optimize_weights_safe(
                rets_win=rets_win,
            ...
            if isinstance(w_opt_raw, pd.Series):
                ...
                w_opt = pd.Series(list(w_opt_raw), index=cols_all, dtype=float)
            ...
            cost_pen = float(to) * (float(tcost_bps) / 10000.0)

            rp_opt = _port_rets(rets_win, w_opt)
            if rp_opt.empty:
                st.error("Portfolio returns vuote dopo optimization.")
                return

            # penalità transazione: approssimo togliendo il costo al primo giorno
    ...
    try:
        ...
        opt_res = st.session_state.get(opt_key)
    ...
    to = float(opt_res.get("turnover", float("nan")))
    cost_pen = float(opt_res.get("cost_pen", 0.0))
    turnover_cap_val = float(opt_res.get("turnover_cap", 0.0))
    ...
    with c2:
        st.metric("Cost penalty (approx)", f"{cost_pen * 100.0:.2f}%")
    with c3:
    ...
    st.dataframe(dfw, use_container_width=True, height=420)

    st.markdown("### 📈 Equity comparison (Base=100)")
    _plot_compare_base100(eq_cur, eq_opt, "Current", "Optimized")

    st.markdown("### ➕ Add optimized to comparison")
    cA, cB = st.columns([1.2, 2.0])
    with cA:
        add_now = st.button("Add optimized", use_container_width=True, key=f"add_opt__{pid}")
    with cB:
